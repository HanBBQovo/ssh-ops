...
func printConfigUsage(w io.Writer) {
	fmt.Fprintln(w, "用参数化命令管理 ssh-ops 配置，尽量不要手改 YAML。")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "用法：")
	fmt.Fprintln(w, "  sshctl config <subcommand> [flags]")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "子命令：")
	fmt.Fprintln(w, "  path         查看当前生效的配置文件路径")
	fmt.Fprintln(w, "  init         创建一个初始配置文件")
	fmt.Fprintln(w, "  show         以 JSON 查看当前配置")
	fmt.Fprintln(w, "  add-host     新增一台服务器")
	fmt.Fprintln(w, "  set-host     创建或更新一台服务器")
	fmt.Fprintln(w, "  remove-host  删除一台服务器")
	fmt.Fprintln(w, "  rename-host  重命名服务器别名或显示名称")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "示例：")
